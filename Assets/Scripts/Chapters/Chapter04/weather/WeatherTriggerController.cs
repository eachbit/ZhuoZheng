...
public class WeatherTriggerController : MonoBehaviour
{
    ...
    private void CreateRainEffect()
    {
        ...
        if (rainPrefab != null)
        {
            ...
            // 确保所有粒子系统都设置为可控制
            ParticleSystem[] particleSystems = rainInstance.GetComponentsInChildren<ParticleSystem>();
            foreach (var ps in particleSystems)
            {
                var main = ps.main;
                main.loop = true; // 确保循环播放，由我们控制何时停止
                Debug.Log($"[WeatherTrigger] 配置粒子系统: {ps.name}, loop={main.loop}");
            }
    ...
    /// 清理场景中可能存在的旧雨效
    /// </summary>
    private void CleanupExistingRain()
    {
        Debug.Log("[WeatherTrigger] 🔍 开始清理场景中可能存在的雨效...");
        
        // 使用 FindObjectsOfType 查找所有 GameObject
        GameObject[] allObjects = FindObjectsOfType<GameObject>(true); // true 包括 inactive 的对象
        int cleanedCount = 0;
        ...
        foreach (var obj in allObjects)
        {
            ...
            // 检查对象名称是否包含雨效相关关键字
            if (obj.name.Contains("Rain") || obj.name.Contains("雨"))
            {
                // 检查是否是Particle System或包含Particle System
                if (obj.GetComponent<ParticleSystem>() != null || 
                    obj.GetComponentInChildren<ParticleSystem>() != null)
                {
                    Debug.Log($"[WeatherTrigger] 发现旧的雨效对象: '{obj.name}' (位置: {obj.transform.position})，正在清理...");
        ...
        if (cleanedCount > 0)
        {
            Debug.Log($"[WeatherTrigger] ✅ 已清理 {cleanedCount} 个旧的雨效对象");
        }
        else
        {
            Debug.Log("[WeatherTrigger] 未发现需要清理的旧雨效对象");
        }
    }
    ...
    private IEnumerator StopRainAfterDelay(float delay)
    {
        ...
        Debug.Log("[WeatherTrigger] 🌤️ 时间到，开始停止雨效并恢复晴天！");
        ...
        {
            Debug.Log($"[WeatherTrigger] 正在处理雨效对象: {rainInstance.name}");
            
            // 先尝试停止所有粒子系统
            ParticleSystem[] particleSystems = rainInstance.GetComponentsInChildren<ParticleSystem>(true);
            Debug.Log($"[WeatherTrigger] 找到 {particleSystems.Length} 个粒子系统");
            
            foreach (var ps in particleSystems)
            {
                ps.Stop(true, ParticleSystemStopBehavior.StopEmittingAndClear);
                Debug.Log($"[WeatherTrigger] 已停止并清除粒子系统: {ps.name}");
            }
            ...
            rainInstance = null;
            Debug.Log("[WeatherTrigger] ✅ 雨效对象已销毁");
        }
        else
        {
            Debug.LogWarning("[WeatherTrigger] ⚠️ 警告：rainInstance 为 null，将执行全局清理！");
        }
        
        // 无论rainInstance是否为null，都执行全局清理，确保没有遗漏的雨效
        Debug.Log("[WeatherTrigger] 执行全局雨效清理...");
        CleanupAllRainEffects();
        
        // 等待一帧后恢复天色
        yield return null;
        
        // 恢复天色为晴天
        Debug.Log("[WeatherTrigger] ☀️ 开始恢复晴天...");
        StartCoroutine(RestoreSkyColor());
        
        Debug.Log("[WeatherTrigger] 🌤️ 雨已完全停止，天色正在恢复！");
    }
    
    /// <summary>
    /// 全局清理所有雨效对象
    /// </summary>
    private void CleanupAllRainEffects()
    {
        // 查找场景中所有 ParticleSystem 组件
        ParticleSystem[] allParticleSystems = FindObjectsOfType<ParticleSystem>(true);
        int stoppedCount = 0;
        ...
        foreach (var ps in allParticleSystems)
        {
            ...
            // 检查是否是雨效相关的粒子系统
            if (obj.name.Contains("Rain") || obj.name.Contains("雨") || 
                obj.name.Contains("rain") || obj.name.Contains("Particle"))
            {
                // 跳过当前触发器及其子对象
                if (obj == gameObject || obj.transform.IsChildOf(transform)) continue;
                
                Debug.Log($"[WeatherTrigger] 发现雨效粒子系统: '{obj.name}'，正在停止...");
        ...
        Debug.Log($"[WeatherTrigger] 全局清理完成：停止了 {stoppedCount} 个粒子系统，销毁了 {destroyedCount} 个对象");
    }
